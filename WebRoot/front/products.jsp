...
<!-- Sidebar end=============================================== -->
    ...
    <ul class="breadcrumb">
		...
		<li class="active">${requestScope.classname}</li>
    </ul>
	<h3>${requestScope.classname1}<small class="pull-right"> 40 products are available </small></h3>	

	<hr class="soft"/>
	<form class="form-horizontal span6" action="${cpath}/servlet/FrontServlet?task=product" method="post">
...
<div class="tab-content">
	...
	<c:forEach var="iteminfo" items="${requestScope.itemlist}">
			...
			<img src="${cpath}/uploadfiles/${iteminfo.filepath}" alt=""/>
			</c:if>
		    <c:if test="${iteminfo.filepath == null || iteminfo.filepath == ''}">
			<img src="${cpath}/images/noPHOTO.png" alt=""/>
			</c:if>
			</div>
			<div class="span4">
