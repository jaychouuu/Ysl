...
<c:param name="menu_id" value="208"></c:param>
</c:import>
...
function changeData(userid){	
	...
	var freezeURL = "<%=path%>/servlet/UserServlet?task=isadmin&userid="+userid;
	var refreshUR = "<%=path%>/servlet/UserServlet?task=usermanage;"
	  $.post(freezeURL,function(jsonData){
			...
			window.alert("操作成功");
			window.location.href="<%=path%>/servlet/UserServlet?task=adminmanage";		
...
<!-- dcHead 结束 --> 
 <div id="dcMain">
   <!-- 当前位置 -->
<div id="urHere">DouPHP 管理中心<b>></b><strong>系统用户管理</strong> </div> 
   ...
   <td align="left" ><input name="submit" id="submit" class="btnGray" type="submit" value="查询" />
    ...
    <table width="100%" border="0" cellpadding="8" cellspacing="0" class="tableBasic">
      <tr>
       ...
       <th width="80" align="center">权限</th>
      ...
      <c:forEach var="userBean" items="${requestScope.userlist}">
        ...
        <td align="center" id="is_admin${userBean.userid}" name="is_admin${userBean.userid}">${userBean.is_admin == 0 ? "管理员":"普通用户"}  </td>
        

        <td align="center">
        <a href="javascript:void(0);"    id="edit${userBean.userid}" name="edit${userBean.userid}"  onclick="changeData(${userBean.userid});">${userBean.is_admin == 0 ? "取消权限":"提升权限"} </a> 
        </td>
      </tr>
       </c:forEach>
