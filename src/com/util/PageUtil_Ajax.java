...
public class PageUtil_Ajax {
	...
	public String createPageTool(String flag) {
		...
		if (flag.equals(PageUtil_Ajax.Text)) {
			...
			str
					.append("<table width='100%' border='0' cellspacing='0' cellpadding='0' style='font-size:12px;'>");
			str.append("<tr>");
			...
			if (this.currentPage > 1) {
				// str.append("<a href='" + url + "&currentPage=1'>首页</a>");
				str
						.append("<a href='javascript:void(0);' onclick=\"loadData(1);\">首页</a>");
				str.append("&nbsp;&nbsp;");
			}
			// 设定是否有上一页的链接
			if (this.currentPage > 1) {
				// str.append("<a href='" + url + "&currentPage=" + ProPage
				// + "'>上一页</a>");
				str.append("<a href='javascript:void(0);' onclick=\"loadData("
						+ ProPage + ")\">上一页</a>");
			...
			if (this.pageCount <= 10) {
				for (int i = 1; i <= this.pageCount; i++) {
					if (this.currentPage == i) {
						...
						str
								.append("<a href='javascript:void(0);' onclick=\"loadData("
										+ i + ");\">" + i + "</a>&nbsp;&nbsp;");
